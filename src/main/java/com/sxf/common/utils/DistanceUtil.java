	 ...
	 //输入经纬度，计算两个坐标间的距离
	  public static double getDistance(double j1,double w1,double j2,double w2) {
	    ...
	    public static double calculate(Point2D pointA, Point2D pointB) {
	        ...
	        double radiansAX = Math.toRadians(pointA.getX()); // A经弧庿
	        double radiansAY = Math.toRadians(pointA.getY()); // A纬弧庿
	        double radiansBX = Math.toRadians(pointB.getX()); // B经弧庿
	        double radiansBY = Math.toRadians(pointB.getY()); // B纬弧庿
	        double cos = Math.cos(radiansAY) * Math.cos(radiansBY) * Math.cos(radiansAX - radiansBX)
	                + Math.sin(radiansAY) * Math.sin(radiansBY);
	        double acos = Math.acos(cos); // 反余弦忿
	        return EARTH_RADIUS * acos; // 朿终结枿
	    }
